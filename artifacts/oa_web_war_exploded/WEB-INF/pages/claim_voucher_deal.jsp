                ...
                <div class="panel-body pn">
                    <table id="message-table" class="table admin-form theme-warning tc-checkbox-1">
                        ...
                        <tr class="message-unread">
                            ...
                            <td>
                                <c:if test="${cv.status==status['CLAIMVOUCHER_CREATED'] || cv.status==status['CLAIMVOUCHER_BACK']}">
                                    <a href="/claim_voucher/to_update?id=${cv.id}">修改</a>
                                    <a href="/claim_voucher/submit?id=${cv.id}">提交</a>
                                </c:if>
                                <c:if test="${cv.status==status['CLAIMVOUCHER_SUBMIT'] || cv.status==status['CLAIMVOUCHER_RECHECK']}">
                                    <a href="/claim_voucher/to_check?id=${cv.id}">审核</a>
                                </c:if>
                                <c:if test="${cv.status==status['CLAIMVOUCHER_APPROVED']}">
